...
- ⏱️ **标准番茄钟**：工作 / 短休息 / 长休息完整循环，可配置专注轮数与番茄个数，运行状态实时显示在状态栏。
...
1. **选择预设**：标准番茄钟 / 整蛊模式 / 自定义；选择「自定义」后可自由调整工作时间、短休 / 长休时间、专注轮数与番茄个数，改动会自动保存。
...
- **两阶段冷启动**：阶段一仅加载 Qt 依赖并立即显示启动画面，阶段二在其后再加载主题（`qt_material`）与业务逻辑（`logic` 链）；`pygame` / `keyboard` 等重库进一步延迟到屏保 / 硬核模式首次使用时才导入，冷启动导入耗时从约 1.6s 降至约 0.2s。
...
```text
Pomodoro_BSOD/
├── main.py                 # 入口：启动画面、两阶段冷启动、主窗口装配、主题与语言切换
