...
- **Two-stage cold start**: stage one loads only the Qt dependencies and shows the splash screen immediately; stage two then loads the theme (`qt_material`) and the business logic (`logic` chain) behind it. Heavy libraries such as `pygame` / `keyboard` are further deferred until a screensaver / hardcore mode is first used, cutting the cold-start import time from ~1.6s to ~0.2s.
...
```text
Pomodoro_BSOD/
├── main.py                 # Entry: splash screen, two-stage cold start, main window assembly, theme and language switching
